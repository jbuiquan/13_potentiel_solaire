...
const Map = () => {
  ...
  const loadRegions = (map) => {
    fetch("/data/a-reg2021.json")
      ...
      .then((regionsData) => {
        // Ajout données pour les clusters simulés
        return fetch("/data/ecoles_par_region.json")
          .then((response) => response.json())
          .then((schoolsData) => {
            console.log("Clusters data:", schoolsData);

            // Ajout du nombre d'écoles par région
            regionsData.features.forEach((region) => {
              const regionData = schoolsData.find((school) => school.code === region.properties.reg);
              if (regionData) {
                region.properties.ecoles = regionData.ecoles;
              }
            });

            // Création des points centraux (centroids)
            const centroids = {
            ...
            // Les clusters simulés
            if (!map.getSource("centroids")) {
              map.addSource("centroids", { type: "geojson", data: centroids });
            }
            // Ajout des cercles (fond coloré sous le texte)
            if (!map.getLayer("regions-centroids-circles")) {
              map.addLayer({
                id: "regions-centroids-circles",
                type: "circle",
                source: "centroids",
                paint: {
                  "circle-radius": 14,
                  "circle-color": "rgba(255, 232, 24, 0.88)",
                  "circle-opacity": 0.8,
                },
                filter: ["has", "ecoles"],
              });
            }
            // Ajout nombre d'écoles sur les centroids
            if (!map.getLayer("regions-centroids-labels")) {
              map.addLayer({
                id: "regions-centroids-labels",
                type: "symbol",
                source: "centroids",
                layout: {
                  "text-field": "{ecoles}",
  ...
  const loadDepartments = () => {
    ...
    if (!map) return;
    map.removeLayer("regions-centroids-circles")
    map.removeLayer("regions-centroids-labels")
